import React from "react";

async function getImageFromMedia(uuid: string) {
  try {
    const res = await fetch(
      `https://darkcyan-koala-320694.hostingersite.com/jsonapi/media/image/${uuid}`,
      { cache: "no-store" }
    );

    if (!res.ok) return null;

    const json = await res.json();

    const fileRel =
      json.data.relationships?.field_media_image?.data;

    if (!fileRel) return null;

    const fileRes = await fetch(
      `https://darkcyan-koala-320694.hostingersite.com/jsonapi/file/file/${fileRel.id}`,
      { cache: "no-store" }
    );

    if (!fileRes.ok) return null;

    const text = await fileRes.text();

    try {
      const fileJson = JSON.parse(text);

      return (
        "https://darkcyan-koala-320694.hostingersite.com" +
        fileJson.data.attributes.uri.url
      );
    } catch {
      console.error("Invalid image JSON:", text);
      return null;
    }
  } catch (error) {
    console.error(error);
    return null;
  }
}

async function getCategoryData(category: string, lang: string) {
  const url = lang === "el"
    ? `https://darkcyan-koala-320694.hostingersite.com/el/api/${category}`
    : `https://darkcyan-koala-320694.hostingersite.com/api/${category}`;

  const res = await fetch(url, { cache: "no-store" });
  
  if (!res.ok) {
    const fallback = await fetch(
      `https://darkcyan-koala-320694.hostingersite.com/api/${category}`,
      { cache: "no-store" }
    );
    if (!fallback.ok) return [];
    return fallback.json();
  }
  return res.json();
}

export default async function Page({ params }: any) {
  const { slug, lang } = await params;

  const data = await getCategoryData(slug, lang);
  

  const items = await Promise.all(
    data.map(async (item: any, i: number) => {
      const title = item.title?.[0]?.value;
      const uuid = item.uuid?.[0]?.value;
      const mediaUUID = item.field__image?.[0]?.target_uuid;
      let image = null;
      if (mediaUUID) image = await getImageFromMedia(mediaUUID);
      return { id: i, title, uuid, image };
    })
  );

  return (
    <main style={{ padding: 40 }}>
      <h1 style={{ marginBottom: 30 }}>
        {slug.replaceAll("-", " ").toUpperCase()}
      </h1>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "30px" }}>
        {items.map((item) => (
          <a href={`/${lang}/node/${item.uuid}`} key={item.id} style={{ textDecoration: "none", color: "inherit" }}>
            <div style={{ overflow: "hidden" }}>
              {item.image && (
                <img src={item.image} style={{ width: "50%" }} alt={item.title} />
              )}
              <div style={{ padding: 15 }}>
                <h3 style={{ margin: 0 }}>{item.title}</h3>
              </div>
            </div>
          </a>
        ))}
      </div>
    </main>
  );
}
